...
import { buildSearchUrl, buildVideoUrl, detectPageKind, sleep } from "./search";

const HOST_PATTERNS = [/\.kuaishou\.com$/i, /^https:\/\/www\.kuaishou\.com/i];
...
async function scrollComments(rounds: number) {
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    const panels = Array.from(
      document.querySelectorAll<HTMLElement>('[class*="comment"], [class*="Comment"]'),
    );
...
export const kuaishouAdapter: PlatformAdapter = {
  ...
  async handleCommand(action: string, payload: unknown) {
    switch (action) {
      ...
      case "kuaishou.page.detect":
        return {
          platform: "kuaishou",
          pageKind: detectPageKind(location.href),
          url: location.href,
        };
      case "kuaishou.search.navigate": {
        const keyword = String((payload as { keyword?: string })?.keyword ?? "").trim();
        if (!keyword) throw new Error("kuaishou.search.navigate: missing keyword");
        return applyNavigate(buildSearchUrl(keyword));
      }
      case "kuaishou.video.navigate": {
        const body = payload as { video_url?: string; photo_id?: string; aweme_id?: string };
        const videoUrl = String(body.video_url ?? "").trim();
        const photoId = String(body.photo_id ?? body.aweme_id ?? "").trim();
        if (videoUrl) return applyNavigate(videoUrl);
        if (photoId) return applyNavigate(buildVideoUrl(photoId));
        throw new Error("kuaishou.video.navigate: missing video_url or photo_id");
      }
      case "kuaishou.comments.scroll": {
      ...
      default:
        throw new Error(`kuaishou adapter: unknown action ${action}`);
    }
  },
};
